...
class Solution {
    public String maskPII(String S) {
        if(S.indexOf('@')!=-1&&S.indexOf('.')!=-1)//S is Email address!
        {
            int theOrderOfA = S.indexOf('@');
            return "" + Character.toLowerCase(S.charAt(0)) + "*****" + Character.toLowerCase(S.charAt(theOrderOfA-1)) + S.substring(theOrderOfA).toLowerCase() ;
        }else
        {
            int count = 0;
            ...
            for (int i = S.length() -1 ; i >= 0 ;i--)
            {
                ...
                {
                    if(count<4)stringBuilder.append(S.charAt(i));
                    else if((count-1)%3 == 0) stringBuilder.append("-*");
                    else stringBuilder.append("*");
                    count++;
                }
            }
            if(count >= 11) stringBuilder.append("+");
